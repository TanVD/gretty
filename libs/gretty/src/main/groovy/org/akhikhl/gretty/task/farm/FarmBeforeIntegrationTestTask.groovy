...
import org.akhikhl.gretty.farm.FarmConfigurer
    ...
    FarmBeforeIntegrationTestTask() {
        ...
        doFirst {
            getIntegrationTestProjects().each { proj ->
                proj.tasks.each { t ->
                    if (GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.app.AppBeforeIntegrationTestTask') ||
                            GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.app.AppAfterIntegrationTestTask')) {
                        if (t.enabled) {
                            t.enabled = false
                        }
                    }
            ...
            project.subprojects.each { proj ->
                proj.tasks.each { t ->
                    if (GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.app.AppBeforeIntegrationTestTask') ||
                            GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.app.AppAfterIntegrationTestTask') ||
                            GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.farm.FarmBeforeIntegrationTestTask') ||
                            GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.farm.FarmAfterIntegrationTestTask') ||
                            GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.farm.FarmIntegrationTestTask')) {
                        if (t.enabled) {
                            t.enabled = false
                        }
                    }
        ...
        project.rootProject.allprojects.each { proj ->
            proj.tasks.all { Task t ->
                if (getIntegrationTestProjects().contains(proj)) {
                    ...
                    } else if (GradleUtils.instanceOf(t, 'org.akhikhl.gretty.task.app.AppBeforeIntegrationTestTask') && t.integrationTestTask == thisTask.integrationTestTask) {
                        t.mustRunAfter thisTask
