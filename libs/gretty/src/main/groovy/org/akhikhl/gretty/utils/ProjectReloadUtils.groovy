...
class ProjectReloadUtils {
    private static final Logger logger = LoggerFactory.getLogger(ProjectReloadUtils)

    private static void addReloadDirs(List<FileReloadSpec> result, Project proj, List reloadSpecs) {
        logger.debug 'addReloadDirs reloadSpecs: {}', reloadSpecs
            ...
            if (spec instanceof String) {
                ...
                spec.each { key, value ->
                    if (key == 'baseDir') {
                        ...
                        logger.warn 'Unknown reload property: {}', key
                    }
                }
                if (!baseDir) {
                    logger.warn 'Reload property baseDir is not specified.'
                    continue
                }
            } else {
                logger.warn 'Reload argument is {}. It must be String, File or Map.', spec?.getClass()?.getName()
                continue
            }
            logger.debug 'addReloadDirs baseDir: {}', baseDir
    ...
    static List<FileReloadSpec> getFastReloadSpecs(Project project, List reloadSpecs) {
        ...
        logger.debug '{} reloadSpecs: {}', project, result
        return result
    }
