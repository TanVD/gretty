...
import org.akhikhl.gretty.farm.FarmConfigurerUtil
import org.akhikhl.gretty.farm.FarmExtension
import org.akhikhl.gretty.farm.FarmWebappType
import org.akhikhl.gretty.farm.FarmsExtension
import org.akhikhl.gretty.servlet.ServletContainerConfig
import org.akhikhl.gretty.task.app.AppAfterIntegrationTestTask
import org.akhikhl.gretty.task.app.AppBeforeIntegrationTestTask
import org.akhikhl.gretty.task.app.AppRestartTask
import org.akhikhl.gretty.task.app.AppStartTask
import org.akhikhl.gretty.task.app.AppStopTask
import org.akhikhl.gretty.task.farm.FarmAfterIntegrationTestTask
import org.akhikhl.gretty.task.farm.FarmBeforeIntegrationTestTask
import org.akhikhl.gretty.task.farm.FarmIntegrationTestTask
import org.akhikhl.gretty.task.farm.FarmRestartTask
import org.akhikhl.gretty.task.farm.FarmStartTask
import org.akhikhl.gretty.task.farm.FarmStopTask
import org.akhikhl.gretty.task.jetty.JettyStartTask
import org.akhikhl.gretty.task.tomcat.TomcatStartTask
import org.akhikhl.gretty.utils.GradleUtils
...
class GrettyPlugin implements Plugin<Project> {
    ...
    private static void addExtensions(Project project) {
    ...
    private static void addTaskDependencies(Project project) {
        ...
        project.tasks.whenObjectAdded { task ->
            if (GradleUtils.instanceOf(task, 'org.akhikhl.gretty.task.app.AppStartTask')) {
                task.dependsOn {
            ...
            } else if (GradleUtils.instanceOf(task, 'org.akhikhl.gretty.task.farm.FarmStartTask')) {
                task.dependsOn {
                    ...
                    if (type in [FarmWebappType.WAR_FILE, FarmWebappType.WAR_DEPENDENCY]) {
                        if (options.overlays) {
                            ...
                            if (type == FarmWebappType.WAR_FILE) {
                                ...
                                warFile = ProjectUtils.getFileFromConfiguration(project, ProjectUtils.getFarmConfigurationName(farmName), typeAndResult[1] as String)
                            }
                    ...
                    case FarmWebappType.WAR_DEPENDENCY:
                        ...
                        if (overlays) {
                            ...
                            if (type == FarmWebappType.WAR_FILE) {
                                ...
                                warFile = ProjectUtils.getFileFromConfiguration(project, ProjectUtils.getFarmConfigurationName(fname), typeAndResult[1] as String)
                            }

                            def outputFolder = Paths.get(project.buildDir.absolutePath, 'farms' as String, fname, 'explodedWebapps', FilenameUtils.removeExtension(warFile.name)).toFile()
                            ...
                            def repackagedArchive = Paths.get(project.buildDir.absolutePath, 'farms' as String, fname, 'explodedWebapps', warFile.name).toFile().absolutePath
    ...
    private static void afterAllProjectsEvaluate(Project rootProject) {
